...
% 使用 Box-Muller 生成高斯白噪声
N = 100000;  % 生成高斯白噪声的长度
x = zeros(1, N);  % 初始化白噪声序列
for i = 1:2:N-1
    U1 = rand();  % 生成均匀分布的随机数 U1
    U2 = rand();  % 生成均匀分布的随机数 U2
    Z1 = sqrt(-2 * log(U1)) * cos(2 * pi * U2);  % Box-Muller 转换
    Z2 = sqrt(-2 * log(U1)) * sin(2 * pi * U2);  % Box-Muller 转换
    x(i) = Z1;  % 将 Z1 存入噪声序列
    if i + 1 <= N
        x(i+1) = Z2;  % 将 Z2 存入噪声序列
    end
end
...
gx_normalized_minus1_1 = gx / max(abs(gx));  % 归一化
