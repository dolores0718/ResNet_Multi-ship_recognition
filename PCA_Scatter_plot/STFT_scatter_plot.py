...
for label in labels:
    ...
    for filename in files:
        ...
        # 提取STFT特征
        # 使用librosa.stft计算STFT
        stft = librosa.stft(signal)

        # 获取STFT的幅度谱
        stft_magnitude = np.abs(stft)

        # 将STFT的幅度谱展平为一维向量并添加到特征集合中
        stft_flat = stft_magnitude.flatten()
...
ax1.set_ylabel('PCA2')
ax1.set_zlabel('PCA3')
ax1.set_title('3D PCA of STFT Magnitude Features with Different Markers')
...
ax2.set_xlabel('PCA1')
ax2.set_ylabel('PCA2')
ax2.set_title('2D PCA of STFT Magnitude Features with Different Markers')
