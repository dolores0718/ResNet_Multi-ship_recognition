...
for label in labels:
    ...
    for filename in files:
        ...
        # 提取log-Mel谱特征
        mel_spec = librosa.feature.melspectrogram(y=signal, sr=sr, n_mels=128)  # 使用y=signal来传递音频信号
        log_mel_spec = librosa.power_to_db(mel_spec)  # 转换为dB（对数Mel谱）

        # 将log-Mel谱展平为一维向量并添加到特征集合中
        log_mel_flat = log_mel_spec.flatten()
        all_features.append(log_mel_flat)
...
ax1.set_ylabel('PCA2')
ax1.set_zlabel('PCA3')
ax1.set_title('3D PCA of log-Mel Spectrogram Features with Different Markers')
...
ax2.set_xlabel('PCA1')
ax2.set_ylabel('PCA2')
ax2.set_title('2D PCA of log-Mel Spectrogram Features with Different Markers')
